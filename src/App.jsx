...
import { Clock, RotateCcw, Sparkles, Zap, HeartPulse, Target, CalendarDays, AlertTriangle, Trophy, Cross, PlusCircle } from "lucide-react";
...
const ROLE_TASKS = {
  student: [
    { id: "club", name: "社團活動", hours: 2, energy: 2, importance: 2, pressure: 2, type: "relationship", description: "開心但也會消耗時間。" },
    { id: "quiz", name: "小考複習", hours: 2, energy: 2, importance: 5, pressure: 5, type: "important", description: "明天就要考。" },
  ],
  worker: [
    { id: "meeting", name: "會議與通勤", hours: 3, energy: 2, importance: 4, pressure: 3, type: "fixed", fixed: true, description: "很難取消的固定行程。" },
    { id: "deadline", name: "專案截止", hours: 3, energy: 4, importance: 5, pressure: 5, type: "important", description: "今天必須推進。" },
  ],
  freelancer: [
    { id: "clientA", name: "客戶 A 交付", hours: 3, energy: 3, importance: 5, pressure: 5, type: "important", description: "最有壓力的交付。" },
    { id: "admin", name: "行政雜事", hours: 2, energy: 1, importance: 2, pressure: 2, type: "routine", description: "不急但容易堆積。" },
  ],
};
...
const CUSTOM_TASK_TYPES = ["important", "health", "relationship", "faith", "temptation", "buffer", "routine"];
...
export default function TimePlannerGamePrototype() {
  ...
  const tasks = useMemo(() => [...BASE_TASKS, ...(ROLE_TASKS[roleId] || []), ...customTasks], [roleId, customTasks]);
  ...
  const addTask = (task) => {
    ...
    setSchedule((prev) => [...prev, { ...task, instanceId: `${task.id}-${Date.now()}-${Math.random()}` }]);
    setShowResult(false);
  };
  ...
  const addCustomTask = () => {
    ...
    setCustomTask({ name: "", hours: 1, type: "routine", importance: 3, pressure: 1, description: "", repeatable: true });
  };
  ...
  const scores = useMemo(() => {
    ...
    const health = clamp((sleepHours >= 7 ? 55 : sleepHours >= 6 ? 40 : 20) + (mealDone ? 25 : 0) + (exerciseDone ? 20 : 0));

    const bufferHours = schedule.filter((s) => s.type === "buffer").reduce((sum, s) => sum + s.hours, 0);
    ...
    const serviceBonus = eventCards.some((e) => e.effect.faithOpportunity) && faithTasks.length > 0 ? 15 : 0;
    const faithPenalty = eventCards.some((e) => e.effect.faithCheck) && faithHours === 0 ? 20 : 0;
    const overTemptationPenalty = schedule.filter((s) => s.type === "temptation").reduce((sum, s) => sum + s.hours, 0) >= 4 ? 10 : 0;
    const faith = clamp(25 + faithHours * 22 + serviceBonus + Math.max(effectiveBuffer, 0) * 4 - faithPenalty - overTemptationPenalty);

    const temptationHours = schedule.filter((s) => s.type === "temptation").reduce((sum, s) => sum + s.hours, 0);
    const relationshipHours = schedule.filter((s) => s.type === "relationship").reduce((sum, s) => sum + s.hours, 0);
    const happinessBonus = eventCards.reduce((sum, e) => sum + (e.effect.happiness || 0), 0);
    const satisfaction = clamp(35 + temptationHours * 10 + relationshipHours * 12 + exerciseDone * 10 + happinessBonus * 5 + Math.min(faithHours, 2) * 6 - Math.max(0, 6 - sleepHours) * 10);
    ...
    const stress = clamp(stressFromMissingImportant + stressFromEvents + stressFromNoBuffer + stressFromOverwork - bufferHours * 8 - Math.min(faithHours, 2) * 8);
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-amber-50 p-4 text-slate-900 md:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border bg-white px-3 py-1 text-sm text-slate-600 shadow-sm">
          ...
          <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {ROLES.map((item) => (
                    ...
                    >
                      ...
                      <div className={`mt-1 text-sm ${roleId === item.id ? "text-slate-200" : "text-slate-500"}`}>{item.description}</div>
                    </button>
                  ))}
                </div>
              </CardContent>
              ...
              <CardContent className="space-y-3 p-5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="space-y-1 text-xs text-slate-500">
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomTask((prev) => ({ ...prev, hours: e.target.value }))}
                  ...
                  <label className="space-y-1 text-xs text-slate-500">
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomTask((prev) => ({ ...prev, importance: e.target.value }))}
                      className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-500"
                    />
                  </label>
                  <label className="space-y-1 text-xs text-slate-500">
                    ...
                    <input
                      ...
                      onChange={(e) => setCustomTask((prev) => ({ ...prev, pressure: e.target.value }))}
                      className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-slate-500"
                    />
                  </label>
                </div>
                ...
                <label className="flex items-center gap-2 text-sm text-slate-600">
                  <input
                    ...
                    onChange={(e) => setCustomTask((prev) => ({ ...prev, repeatable: e.target.checked }))}
                  />
                  可重複安排
                </label>
                ...
                {customTasks.length > 0 && (
                  <div className="space-y-2 border-t pt-3">
                    {customTasks.map((task) => (
                      <div key={task.id} className="flex items-center justify-between gap-2 rounded-2xl bg-slate-50 p-2 text-sm">
                        <span className="truncate">{task.name}</span>
                        <button onClick={() => removeCustomTask(task.id)} className="text-slate-400 hover:text-red-500">刪除</button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <Card className="rounded-3xl border-0 shadow-sm">
              <CardContent className="space-y-3 p-5">
                ...
                <div className="space-y-2">
                  {tasks.map((task) => {
                    const disabled = (!task.repeatable && taskCounts[task.id]) || freeHours < task.hours;
                    return (
                      <button
                        key={task.id}
                        disabled={disabled}
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="shrink-0 rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-600">{task.hours}h</span>
                        </div>
                        <div className="mt-2 flex items-center gap-2">
                          <span className={`rounded-full border px-2 py-0.5 text-xs ${TYPE_STYLE[task.type] || TYPE_STYLE.routine}`}>
                            {TYPE_LABEL[task.type] || "任務"}
                          </span>
                          <span className="text-xs text-slate-400">重要 {task.importance} / 壓力 {task.pressure}</span>
                        </div>
                      </button>
                    );
                  })}
              ...
              <CardContent className="p-5">
                <div className="mb-4 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">已安排 {usedHours} 小時，剩下 {freeHours} 小時。</p>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-8 xl:grid-cols-12">
                  {timelineBlocks.map((item, index) => (
                    ...
                    >
                      <div className="font-bold tabular-nums">{String(index).padStart(2, "0")}:00</div>
          ...
          <div className="space-y-4">
            <Card className="rounded-3xl border-0 shadow-sm">
              <CardContent className="space-y-4 p-5">
                ...
                <Button onClick={drawEvents} disabled={schedule.length === 0} className="w-full rounded-2xl">
                  <Zap className="mr-2 h-4 w-4" /> 抽事件並計算結果
                </Button>
                <div className="space-y-2">
                  {eventCards.length === 0 ? (
                    <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-400">尚未抽事件。</div>
                  ) : (
